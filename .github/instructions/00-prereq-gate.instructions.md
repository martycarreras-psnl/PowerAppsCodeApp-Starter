...
5. **Corporate proxy / SSL inspection.** If `npm install`, `dotnet tool install`, or `pip install` fail with **certificate errors** (`CERTIFICATE_VERIFY_FAILED`, `SSLError`, `unable to get local issuer`), the user is behind an SSL-inspecting proxy. PACAF cannot fix this — the user (or their IT team) must configure `NODE_EXTRA_CA_CERTS`, the .NET cert store, and `pip` trusted hosts. Surface this as the diagnosis; do not loop trying retry flags.
6. **Blocked public package registry (Microsoft-managed devices / CFS).** Distinct from #5 — the symptom here is a **connection refused / DNS failure / HTTP 403** against `api.nuget.org`, `nuget.org/api/v2`, `pypi.org/simple`, or `files.pythonhosted.org`, **not** a certificate error. Microsoft-managed devices block direct access to public NuGet/PyPI (and npm, from an earlier wave) via policy (Central Feed Services); package managers must route through the approved proxy feeds. Do **not** advise `--trusted-host` (that's for #5 and won't help a domain block) and do **not** loop retrying. If the user is on a Microsoft device, the fix is to point the package manager at the CFS proxy — `pip config set global.index-url https://packagefeedproxy.microsoft.io/pypi/simple` and `dotnet nuget add source https://packagefeedproxy.microsoft.io/nuget/v3/index.json -n CFS` (both proxies are Microsoft-internal and unreachable elsewhere). Many managed devices already have this configured by policy, in which case installs just work. If the user is **not** on a Microsoft device, this is their own corporate proxy — direct them to their organization's approved internal feed, never to the Microsoft proxy.
7. **OneDrive-synced workspace path.** If the repo lives under `C:\Users\<you>\OneDrive\…` and OneDrive is set to "Files On‑Demand", `pnpm install` and the wizard's auth scrub can fail with permission errors. The fix is to either move the repo out of OneDrive or set the folder to "Always keep on this device." Detect this from the workspace path.
